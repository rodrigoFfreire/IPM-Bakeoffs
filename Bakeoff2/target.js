...
class Group extends Frame {
  ...
  draw() {
    for (let i = 0; i < this.targets.length; i++) this.targets[i].draw();
  }
}

function loadGroup(menu, targets, regex, table) {
    ...
    if (!seen.has(pref3)) {
      ...
      for (let j = i + 1; j < matches.length; j++)
        if (matches[j].getString(1).substring(0, 3).localeCompare(pref3) === 0)
          count++;
      seen.add(pref3);
      group = new Targets(0, 0, 0, 0, target_size * count + 0.01, target_size, pref3, DEFAULT_MENU_FONT, target_text_size, COLOR_WHITE);
      targets.with(group);
      hue = (hue + 40) % 360;
    }
    let name = matches[i].getString(1);
    name = name.substring(0, 3).toUpperCase() + name.substring(3, name.length - 1) + name.charAt(name.length - 1).toUpperCase();
